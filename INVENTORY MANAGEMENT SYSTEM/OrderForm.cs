...
namespace INVENTORY_MANAGEMENT_SYSTEM
{
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            ...
            int i = 0;
            dgvOrder.Rows.Clear();
            cm = new SqlCommand("SELECT oderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price, total FROM tbOrder AS O JOIN tbCustomer AS C ON O.cid=C.cid JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT (oderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price) LIKE '%"+textSearch.Text+"%' ", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
                ...
                total += Convert.ToInt16(dr[8].ToString());
            }
            dr.Close();
            con.Close();
           ...
           if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbOrder WHERE oderid LIKE'" + dgvOrder.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
